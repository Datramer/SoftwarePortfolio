#this is a modified version of asteroids based on the project found at https://www.youtube.com/watch?v=XKMjMGbdrpY&t=34s 
import pygame
import math
import random
...
playerRocket = pygame.image.load('asteroidsPics/knight.png')
...
bangLargeSound = pygame.mixer.Sound('sounds/scream.wav')
bangSmallSound = pygame.mixer.Sound('sounds/scream.wav')
shoot.set_volume(.25)
...
class Player(object):
    ...
    def moveLeft(self):
        self.xfut = self.x - 5
        self.timetilldash = self.timetilldash - 1
    ...
    def dashLeft(self):
        if (self.timetilldash <= 0):
            self.timetilldash = 100
            self.xfut = self.x - 250
        else:
            self.xfut = self.x - 5
            self.timetilldash = self.timetilldash - 1
...
##############################################################################
# This is where the running loops are located                                #
##############################################################################
while run:
    clock.tick(60)
    count += 1
